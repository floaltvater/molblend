    ...
    def add_bonded_atom(self, ob, replace_name=""):
        ...
        if ob_name in self.bonded_atoms:
            ...
            if item:
                ...
                debug_print("WARNING: {} not found in {}".format(
                            replace_name, self.bonded_atoms), 3)
        else:
    ...
    def remove_bonded_atom(self, ob):
        if not self.type == 'BOND':
            debug_print("WARNING: Something is trying to remove bonded_atom "
                ...
                ob.name, ob.mb.type, self.name, self.type), 1)
            return
...
class mb_molecule(PropertyGroup):
    ...
    # dito
    draw_style = EnumProperty(name="Display style", default='BAS',
    ...
    def add_object(self, ob, replace_name=""):
        ...
        if ob_name in collection:
            ...
            if item:
                ...
                debug_print("WARNING: "
                    "{} not found in {}".format(replace_name, collection), 3)
        else:
            item = collection.add()
            item.name = ob_name
        return item
...
class mb_scn_import(PropertyGroup):
    filepath = StringProperty(name="Import file", default=os.getcwd() + "/", 
        subtype="FILE_PATH",
        description="Filepath to molecule file to import (.xyz, .pdb)")
    modes = BoolProperty(name="Modes", default=False, 
        description="Import normal modes of molecule as keyframes.")
    modes_path = StringProperty(name="Modes file", default="", 
        subtype="FILE_PATH",
...
class mb_wm_globals(PropertyGroup):
    group_selected_extend = BoolProperty(name="Extend", default=False, 
        description="Extend selected group to selection")
    element_to_add = StringProperty(name="Element", 
        default="C", description="Element to add to scene")
    geometry_to_add = EnumProperty(name="Geometry", default='SINGLE', 
